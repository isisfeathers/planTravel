...
describe("useWizardStore", () => {
  ...
  it("sports 可選填 event_note，取消 sports 時會清空備註", () => {
    ...
    expect(useWizardStore.getState().accommodation).toBe("switch_hotel");
    expect(useWizardStore.getState().transit).toBe("public_transit");
    expect(useWizardStore.getState().interests).toEqual([
      "sports",
      "gourmet",
      "shopping",
    ]);

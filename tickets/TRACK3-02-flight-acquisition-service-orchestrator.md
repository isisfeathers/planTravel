...
- [x] **硬性前置實體產出確認**：mock_itinerary.json 的 flight_data 結構、Apify 託管爬蟲 Token。
- [x] **規範與契約理解確認**：清楚 IATA 機場解析、4.5 秒超時預算、去重演算法與加權評分公式。
- [x] **紅線與禁止事項確認**：嚴禁對消費者網站發起任何本地自建 DOM 爬蟲（改串 Apify 託管爬蟲合規安全），超時必須優雅降級不阻塞主行程。
- [x] **執行者開工簽核**：我已完全理解本任務之驗收標準與前置交付物，確認無缺漏並正式開工。
...
- [x] 撰寫 IATA 機場代碼對照庫（支援城市名稱至機場三字碼之雙向轉換）。
- [x] 實作抽象 `FlightProviderAdapter` 介面：
  - [x] 實作 `ApifyAdapter`：呼叫 Apify 雲端 Google Flights 爬蟲服務 (託管合規)。
- [x] 撰寫 `SearchOrchestrator`：
  - [x] 設定超時閥值為 4.5 秒。
  - [x] 平行呼叫 Adapters，聚合各來源資料。
  - [x] 實作 Circuit Breaker 斷路熔斷機制保護外部 API。
- [x] 實作資料正規化與去重演算法：以 `MD5(airline + flight_no + departure_time + arrival_time)` 為鍵進行去重，優先保留含免費托運行李且價低者。
- [x] 實作加權評分公式：
  $$Score = 100 - (PriceNorm \times 40) - (DurationNorm \times 35) - (Stops \times 15) + (BaggageBonus \times 10)$$
- [x] 提供標準 REST 搜尋端點：`GET /api/v1/flights/search`。

---

## 4. 驗收條件 (Acceptance Criteria)
- [x] 輸入「台北」到「東京」，系統能正確轉換為 TPE/TSA 至 NRT/HND 並發送合法 API 請求。
- [x] 產出的資料結構 100% 吻合 `FlightOfferItem`，包含航空公司代碼、航班編號、起降時間與行李額度。
- [x] 航班評分演算法能正確將直飛、含行李且價格合理的航班排於首位。
- [x] 任何第三方連線若超過 4.5 秒，調度器能強制截斷並回傳已取得之資料，絕不拖垮主行程。
